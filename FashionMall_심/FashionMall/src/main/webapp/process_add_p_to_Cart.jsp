...
<html>
	...
	<body>
	<%
	    // p_id_for_addToCart 파라미터 값을 받아옴
	    String p_id_for_addToCart = request.getParameter("p_id");
	
	    // p_id_for_addToCart가 null인지 확인
	    if (p_id_for_addToCart == null) {
	        out.println("상품 ID가 null입니다."); // 값이 null인 경우 출력
	    } else if (p_id_for_addToCart.trim().isEmpty()) {
	        out.println("상품 ID가 비어있습니다."); // 값이 비어있는 경우 출력
	    } else {
	        out.println("상품 ID: " + p_id_for_addToCart); // 정상적으로 값이 있는 경우 출력
	    }

		// DAO 객체 생성
		DAO_SQL dao_sql = DAO_SQL.getInstance();
		
		// 상품 정보를 조회하여 DTO 객체에 저장
		...
		//=======================▼장바구니에 추가할 상품정보 조회 처리 후 해당되는 상품만 추가되는 과정=====================================		
		...
		// 장바구니 추가 후 상품 상세 페이지로 리다이렉트

		response.sendRedirect("FashionMall_P_detail.jsp?p_id=" + p_id_for_addToCart);
	%>
	</body>
</html>
