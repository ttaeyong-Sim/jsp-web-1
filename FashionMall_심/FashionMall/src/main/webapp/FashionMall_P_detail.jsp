	...
	<script type="text/javascript">
		function addProductToCart() {
			if(confirm("상품을 장바구니에 추가하시겠습니까?")) {
				document.shoppingCartForm.submit();  // 폼을 제출하여 장바구니에 상품을 추가
			} else {
				document.shoppingCartForm.reset();  // 취소시 폼 리셋
			}
		}
	</script>
</head>
<body>
	...
	<%
 		String productId = request.getParameter("p_id"); //여기서그냥실행하면 이 파라미터값이 없기때문에 Null 포인터 오류가생김
 	 ...
 	 // System.out.println("전달된 상품고유번호: " + productId);
 		...
 		DTO dto = dao_sql.getProductById(productId);
 		//System.out.println("DTO dto:" + dto);
 		//▲1.model패키지의 DAO_SQL클래스의 참조변수 dao_sql 통해 getProductById메소드 실행 매개변수는 필드의 productId로 설정함
	...
	<div class="row align-items-md-stretch">
		...
		<div class="col-md-6">
			...
			<br>
			 ...
			 <form name ="shoppingCartForm" action="./process_add_p_to_Cart.jsp" method="post">
			 		<input type="hidden" name="p_id" value="<%=dto.getProductId()%>">						
